...
void rainbowCycleTimed() {
  uint16_t i, j;   
  unsigned long currentMillis = millis();
  if (currentMillis - previousMillisRGBs > 10) {
    previousMillisRGBs = currentMillis;    
    for(i=0; i< strip.numPixels(); i++) {
      strip.setPixelColor(i, Wheel(((i * 256 / strip.numPixels()) + LEDrgb) & 255));
    }
    strip.show();
    LEDrgb++;
  }
...
//  resetLEDs();
  unsigned long currentMillis = millis();
  if (currentMillis - previousMillisReds > 300) {
  previousMillisReds = currentMillis;  
    if(flipState == HIGH) {
